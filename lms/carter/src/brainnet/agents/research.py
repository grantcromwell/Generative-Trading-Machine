"""
Research Agent - GAF generation + native vision analysis with Phi-3.5-Vision-Instruct

Uses NVIDIA NIM API - no local GPU required.
Model: microsoft/phi-3.5-vision-instruct

Handles base64 PNGs of GAF images natively for ETHUSDT-PERP scalping.
...
class ResearchAgent(BaseAgent):
    ...
    from time series data and analyzes them using Phi-3.5-Vision-Instruct
    via NVIDIA NIM API.
    
    Model: microsoft/phi-3.5-vision-instruct
    
    Vision mode is enabled by default - handles base64 PNGs natively.
    Optimized for live ETHUSDT-PERP scalping.
    """

    def __init__(self, use_vision: bool = True, **kwargs):
        """
        Initialize ResearchAgent with vision enabled by default.
        
        Args:
            use_vision: Enable vision mode for GAF image analysis (default: True)
            **kwargs: Passed to BaseAgent (backend, api_key, base_url)
        """
        super().__init__(**kwargs)
        self.gaf_summation = GramianAngularField(method='summation')
        self.gaf_difference = GramianAngularField(method='difference')
        # Vision enabled by default - Phi-3.5-Vision handles images natively
        self.use_vision = use_vision if use_vision is not None else os.getenv("USE_VISION", "true").lower() == "true"
    ...
    def generate_gaf_image(
        ...
        Returns:
            Base64-encoded PNG image string (native input for Phi-3.5-Vision)
        """
    ...
    def generate_gaf_3channel_base64(
        ...
        Convenient for web transmission, storage, or direct Phi-3.5-Vision input.
    ...
    def extract_gaf_features(self, series: np.ndarray) -> dict:
        """
        Extract numerical features from GAF matrix for fallback text-based analysis.
        
        Note: With Phi-3.5-Vision, prefer direct image analysis via vision mode.
    ...
    def analyze_pattern(
        ...
        """
        Analyze a GAF pattern using Phi-3.5-Vision-Instruct.
        
        Vision mode (default): Sends base64 PNG directly to the model.
        Fallback text mode: Uses extracted numerical features.

        Args:
            image_base64: Base64-encoded PNG image
            prompt: Custom analysis prompt
            gaf_features: Pre-extracted GAF features (for text mode fallback)

        Returns:
            Analysis text from the model
        """
        base_prompt = """Analyze this Gramian Angular Field (GAF) representation of ETHUSDT-PERP time series data.
...
- Scalping opportunity rating (1-10)

Provide your analysis in a structured format."""

        if self.use_vision:
            # Vision mode - send image directly to Phi-3.5-Vision
            final_prompt = prompt or base_prompt
            return self.llm.generate_with_image(final_prompt, image_base64, max_tokens=1024)
        else:
            # Text mode fallback - use extracted features
...
Analyze these features and provide your pattern assessment."""
    ...
    def research(
        ...
        Uses Phi-3.5-Vision to analyze GAF images natively.

        Args:
            data: DataFrame with 'Close' column (ETHUSDT-PERP OHLCV)
        ...
        # Extract GAF features for metrics/fallback
        gaf_features = self.extract_gaf_features(series)

        # Build analysis prompt with memory context
        base_prompt = """Analyze this Gramian Angular Field (GAF) representation of ETHUSDT-PERP price data.
...
Rate scalping opportunity (1-10) for the next few bars."""
...
Given this context, {base_prompt}"""
        ...
        # Analyze with Phi-3.5-Vision (uses native image understanding)
