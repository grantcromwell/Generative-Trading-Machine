"""
Research Agent - GAF generation + analysis with DeepSeek-V3
Supports both text analysis (primary) and GAF numerical features
...
class ResearchAgent(BaseAgent):
    ...
    from time series data and analyzes them using DeepSeek-V3.
    
    Supports two modes:
    - Text mode (primary): Extracts numerical features from GAF for LLM analysis
    - Vision mode: For models with vision capability (future DeepSeek-VL support)
    """
    ...
    def __init__(self, use_vision: bool = None, **kwargs):
        ...
        # DeepSeek-V3 is text-only, default to text mode
    ...
    def analyze_pattern(
        ...
        """
        Analyze a GAF pattern using DeepSeek-V3.
        Uses text mode with extracted features (DeepSeek-V3 is text-only).

        Args:
            image_base64: Base64-encoded PNG image (for storage/reference)
            prompt: Custom analysis prompt
            gaf_features: Pre-extracted GAF features (required for text mode)
...
Provide your analysis in a structured format."""
        ...
        # DeepSeek-V3 is text-only - always use extracted features
