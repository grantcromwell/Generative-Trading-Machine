...
from brainnet.services.data_flywheel import DataFlywheel
from brainnet.services.market_data import MarketDataService
...
def run_single_analysis(symbol: str = "ES=F", interval: str = "5m") -> dict:
    ...
    market_data = MarketDataService(
        ...
        coinglass_api_key=config.get("coinglass_api_key", "")
    )
    
    # Fetch data with derivatives metrics
    ...
    if config.get("use_coinglass", True):
        ...
        derivatives = market_data.get_derivatives_metrics(symbol)
        if derivatives:
    ...
    flywheel.collect_trading_data(result)
    print("    ✓ Collected in Data Flywheel")
